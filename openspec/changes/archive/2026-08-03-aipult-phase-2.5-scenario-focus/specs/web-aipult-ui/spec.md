...
dashboard и подсвечивать соответствующую карточку сценария.

#### Scenario: ?focus=<id> highlights matching card

WHEN dashboard открывается с URL `?tab=rendered&focus=b16e0660`,
THEN:
1. Активируется таб `rendered`
2. После рендера карточек, `.card[data-scenario-id="b16e0660"]` SHALL получить class `card--focused`
3. Карточка SHALL scroll into view (`scrollIntoView({behavior: 'smooth', block: 'center'})`)
4. Через 3000ms class `card--focused` SHALL быть удалён (auto-cleanup)
5. `?focus=` SHALL быть удалён из URL через `history.replaceState` чтобы reload не re-triggered highlight

#### Scenario: ?focus=<nonexistent> silently fails

WHEN `?focus=NONEXISTENT` указан, но карточка с таким ID не найдена в DOM,
THEN ничего не подсвечивается, no error logged, no console message.

#### Scenario: data-scenario-id is present on every scenario card

WHEN `scenarioCard(sc, status)` рендерит карточку,
THEN HTML MUST contain `data-scenario-id="<sc.id>"` attribute.
